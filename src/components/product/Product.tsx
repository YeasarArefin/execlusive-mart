...
export default function Product({ product }: { product: ProductType; }) {
    ...
    useEffect(() => {
        ...
        if (colorParam) {
            const matchedColor = colors.find(color => color.color_name.toLowerCase() === colorParam.toLowerCase());
            if (matchedColor) {
                setSelectedColor(matchedColor);
                // Find the image index that matches this color
                const colorIndex = colors.findIndex(color => color.color_name.toLowerCase() === colorParam.toLowerCase());
                if (colorIndex !== -1) {
        ...
        if (sizeParam) {
            const matchedVariant = variants.find(variant => variant.size.toLowerCase() === sizeParam.toLowerCase());
            if (matchedVariant) {
                setSelectedVariant(matchedVariant);
            }
        }
    }, [searchParams, colors, variants]);
                ...
                <div className="flex flex-col lg:flex-row gap-12">
                    ...
                    <div className="flex flex-col lg:flex-row gap-4 lg:w-1/2 w-full">
                        ...
                        <div className="w-full h-[250px] sm:h-[300px] md:h-[350px] lg:h-[425px] rounded-xl order-1 lg:order-2">
                            <div className={`relative w-full h-full transition-opacity duration-300 ${fade ? 'opacity-0' : 'opacity-100'}`}>
                                <InnerImageZoom
                                    zoomType="hover"
                    ...
                    <div className="lg:w-1/2 w-full px-5">
                                        ...
                                        <TooltipProvider>
                                            <Tooltip>
                                                <TooltipTrigger asChild>
                                                    <span
                                                        onClick={() => { setSelectedColor(color); handleChangeImageIndex(idx); }}
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            ...
                            <div className="flex gap-x-5 items-center">
                                <AddToCart product={product} color={selectedColor} variant={selectedVariant} quantity={productQuantity} className='px-3 py-5' />
